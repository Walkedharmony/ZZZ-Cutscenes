...
namespace ZZZCutscenes.Mergers.ZZZMKV.MKV.Elements.Cluster
{
    internal class Cluster
    {
        ...
        public byte[] ToBytes()
        {
            ...
            byteRes.InsertRange(0, ZZZMKV.FieldLength((uint)byteRes.Count));
        ...
        public bool CanBeAdded(int finalTimestamp, uint totalDataSize, int videoFrames = 0, int audioFrames = 0, int subsFrames = 0) => (finalTimestamp - timestamp) < ZZZMKV.ClusterTimeLength && (totalDataSize + currentSize + 9 * videoFrames + 10 * audioFrames + 10 * subsFrames) < ZZZMKV.ClusterSize;
