...
namespace ZZZCutscenes.Mergers.ZZZMKV.MKV.Elements.Attachment
{
    internal class AttachedFile
    {
        ...
        public uint Length()
        {
            ...
            _firstFields.AddRange(ZZZMKV.FieldLength((uint)_file.Length));

            uint length = (uint) (_firstFields.Count + _file.Length);
            _firstFields.InsertRange(0, ZZZMKV.FieldLength(length));
